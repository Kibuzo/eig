...
namespace Server.Misc
{
	...
	public static double erf(double x)
	{
		...
		while (i<100)
		{
			double den=(Factorial(i)*(2*i+1));
			a+=(2/Math.Sqrt(Math.PI))*(Math.Pow(-1,i)*Math.Pow(x,(2*i+1)))/den;
			i+=1;
		}
		return(a);
	}
	...
	public static double Gaussian(double xmu,double sigma)
	{
		double g=0;
		g=(1/Math.Sqrt(2*Math.PI*Math.Pow(sigma, 2)))*Math.Pow(2.71, (-Math.Pow(xmu,2))/(2*Math.Pow(sigma,2)) );
		return (g);
	}
        ...
        public static bool CheckSkill(Mobile from, Skill skill, object amObj, double chance) //TODO: aggiustare questa schifezza: gain chance è sempre la stessa, mi imbarazza avere sul computer un tale schifo
        {
            if (from.Skills.Cap == 0)
                return false;
	   //bisognerebbe tirare un valore random e vedere se cumulativa(skill) è maggiore del random, nel caso è un successo
            double x = Utility.RandomDouble();
			double cum = Cumulative(chance-0.5,0.1);
            bool success = (cum >= x); //Utility.RandomDouble());
            //from.SendMessage("Chance was {0}", chance); //debug
	        //from.SendMessage("Cumulative was {0}",cum); //moar debug
			//from.SendMessage("Success was {0}", cum-x);
	    	double sigma=0.5; //definisci la varianza
            double gc=Gaussian(chance-0.5, sigma);
	    	//from.SendMessage("Final gc={0}\n", gc); //debug
